    ...
    if (typeof $.fn.btnAddRow == "undefined")
    {
        ...
        ExpandableTable.prototype.addRow = function ()
        {
            ...
            if (!this.maxRow || (this.maxRow && $("." + this.cloneClass).size() < this.maxRow))
            {
                ...
                this.newRow = newRow = lastRow.clone().fadeIn("fast");
                //newRow.find("span[id$=rowNumber]").html($("." + this.cloneClass).size() + 1);
                newRow.find('select[id$=txt_Year]').get(0).selectedIndex = 88;
                newRow.find("input[type=text][id$=txt_Desc]").val("");
                newRow.find("input[type=text][id$=txt_DocNumber]").val("");
                newRow.find("input[type=text][id$=txt_Amount]").val("0");
