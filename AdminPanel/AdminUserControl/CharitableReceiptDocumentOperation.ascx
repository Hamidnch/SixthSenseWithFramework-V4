<%@ Control Language="C#" AutoEventWireup="true" CodeFile="CharitableReceiptDocumentOperation.ascx.cs" Inherits="AdminPanel_AdminUserControl_CharitableReceiptDocumentOperation" %>
...
<fieldset>
    <legend style="color: #ffcc00;">لیست اسناد دریافت - صندوق عام المنفعه</legend>
    <fieldset>
    ...
    <asp:UpdatePanel ID="UpdatePanel1" runat="server" UpdateMode="Conditional">
        <ContentTemplate>
            <fieldset>
                <asp:GridView ID="gv_CharitableReceiptDocuments" runat="server" AutoGenerateColumns="False"
                    CssClass="GridViewStyle" HeaderStyle-CssClass="HeaderStyle" 
                    HeaderStyle-HorizontalAlign="Center" HeaderStyle-VerticalAlign="Middle" 
                    AllowPaging="True" 
                    onpageindexchanging="gv_CharitableReceiptDocuments_PageIndexChanging" 
                    onrowcommand="gv_CharitableReceiptDocuments_RowCommand" 
                    onrowdatabound="gv_CharitableReceiptDocuments_RowDataBound">
...
<%--<input type="button" value="ddd" onclick="__doPostBack($('input[id$=btn_Refersh]').attr('id'), null);" />--%>
...
<script type="text/javascript">
    ...
    function getCharitableReceiptInfo()
    {
        $.PageMethod('GetCharitableReceiptInfo', onCharitableReceiptInfoSuccess, onCharitableReceiptInfoFail);
        function onCharitableReceiptInfoSuccess(result)
        {
            var info = result.d;
        ...
        function onCharitableReceiptInfoFail(response)
        {
            alert("خطا");
            $.unblockUI();
        }
    }
    function addAmountToCharitableBoxByReceipt(amount)
    {
        setBlock();
        $.PageMethod('AddAmountToCharitableBoxByReceipt', onAddAmountSuccess, onAddAmountFail, 'amount', amount);
    ...
    $(document).ready(function ()
    {
        ...
        $("#dialog-receipt").dialog({
            ...
            show: 'clip', hide: 'clip', stack: true, position: 'center', title: 'سند دریافت - صندوق عام المنفعه', modal: true
        });
        ...
        $('#register').click(function ()
        {
            ...
            $(".autoTable tbody tr").each(function (item, i)
            {
                ...
                saveReceipt(receiptID, "3", oldAmount, n, receiptDate, txt_Desc, txt_DocNumber, txt_Amount, txt_comment, "-1");
                $('span[id$=CurrentRemainAmountBox]').html(parseFloat(oldAmount) + parseFloat(txt_Amount));
                remainAmount += parseFloat(txt_Amount);
            });
            addAmountToCharitableBoxByReceipt(sumAmount);
