<%@ Control Language="C#" AutoEventWireup="true" CodeFile="CharitablePaymentDocumentOperation.ascx.cs" Inherits="AdminPanel_AdminUserControl_CharitablePaymentDocumentOperation" %>
...
<fieldset>
    <legend style="color: #ffcc00;">لیست اسناد پرداخت - صندوق عام المنفعه</legend>
    <fieldset>
    ...
    <asp:UpdatePanel ID="UpdatePanel1" runat="server" UpdateMode="Conditional">
        <ContentTemplate>
            <fieldset>
                <asp:GridView ID="gv_charitablePaymentDocuments" runat="server" AutoGenerateColumns="False"
                    CssClass="GridViewStyle" HeaderStyle-CssClass="HeaderStyle" 
                    HeaderStyle-HorizontalAlign="Center" HeaderStyle-VerticalAlign="Middle" 
                    AllowPaging="True" 
                    onpageindexchanging="gv_charitablePaymentDocuments_PageIndexChanging" 
                    onrowcommand="gv_charitablePaymentDocuments_RowCommand" 
                    onrowdatabound="gv_charitablePaymentDocuments_RowDataBound">
...
<script type="text/javascript">
    ...
    function getCharitablePaymentInfo()
    {
        $.PageMethod('GetCharitablePaymentInfo', onCharitablePaymentInfoSuccess, onCharitablePaymentInfoFail);
        function onCharitablePaymentInfoSuccess(result)
        {
            var info = result.d;
        ...
        function onCharitablePaymentInfoFail(response)
        {
            alert("خطای برگرداندن اطلاعات");
            $.unblockUI();
        }
    }
    function reduceAmountFromCharitableBoxByPayment(amount)
    {
        setBlock();
        $.PageMethod('ReduceAmountFromCharitableBoxByPayment', onReduceAmountSuccess, onReduceAmountFail, 'amount', amount);
    ...
    $(document).ready(function ()
    {
        ...
        $("#dialog-Payment").dialog({
            ...
            show: 'clip', hide: 'clip', stack: true, position: 'center', title: 'سند پرداخت - صندوق عام المنفعه', modal: true
        ...
        $('#register').live("click", function (event)
        {
            ...
            if (currentAmount >= sumAmount)
            {
                $(".autoTable tbody tr").each(function (item, i)
                {
                    ...
                    //                    alert(txt_Amount);
                    savePayment(PaymentID, "3", oldAmount, n, PaymentDate, txt_Desc, txt_DocNumber, txt_Amount, txt_comment, "-1");
                    $('span[id$=CurrentRemainAmountBox]').html(parseFloat(oldAmount) - parseFloat(txt_Amount));
                    remainAmount -= parseFloat(txt_Amount);
                });
                reduceAmountFromCharitableBoxByPayment(sumAmount);
