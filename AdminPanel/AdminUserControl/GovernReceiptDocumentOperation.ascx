<%@ Control Language="C#" AutoEventWireup="true" CodeFile="GovernReceiptDocumentOperation.ascx.cs" Inherits="AdminPanel_AdminUserControl_GovernReceiptDocumentOperation" %>
...
<fieldset>
    <legend style="color: #ffcc00;">لیست اسناد دریافت - صندوق دولتی</legend>
    <fieldset>
    ...
    <asp:UpdatePanel ID="UpdatePanel1" runat="server" UpdateMode="Conditional">
        <ContentTemplate>
            <fieldset>
                <asp:GridView ID="gv_governReceiptDocuments" runat="server" AutoGenerateColumns="False"
                    ...
                    onpageindexchanging="gv_governReceiptDocuments_PageIndexChanging" 
                    onrowdatabound="gv_governReceiptDocuments_RowDataBound" 
                    onrowcommand="gv_governReceiptDocuments_RowCommand">
...
<input type="button" onclick="<%= btn_Refersh.ClientID %>.click();" id="btnRefreshNext" style="display: none" />
                                ...
                                <tr>
                                    <td colspan="4">
                                        <asp:GridView ID="gv_ShowReceipt" runat="server" AutoGenerateColumns="false"  CssClass="GridViewStyle" HeaderStyle-CssClass="HeaderStyle" PagerStyle-CssClass="PagerStyle">                        
                                            ...
                                            <EmptyDataTemplate>
                                                <asp:Label ID="Label33" runat="server" Text="هیچ موردی یافت نشد" Width="100%"></asp:Label>
                                            </EmptyDataTemplate>
                                        </asp:GridView>
                                    </td>
                                </tr>
...
<script type="text/javascript">
    ...
    function getGovernReceiptInfo()
    {
        $.PageMethod('GetGovernReceiptInfo', onGovernReceiptInfoSuccess, onGovernReceiptInfoFail);
        function onGovernReceiptInfoSuccess(result)
        {
            var info = result.d;
        ...
        function onGovernReceiptInfoFail(response)
        {
            alert("خطا");
            $.unblockUI();
        }
    }
    function addAmountToGovernBoxByReceipt(amount)
    {
        setBlock();
        $.PageMethod('AddAmountToGovernBoxByReceipt', onAddAmountSuccess, onAddAmountFail, 'amount', amount);
    ...
    $(document).ready(function ()
    {
        ...
        $("#dialog-receipt").dialog({
            ...
            show: 'clip', hide: 'clip', stack: true, position: 'center', title: 'سند دریافت - صندوق دولتی', modal: true
        });
        ...
        $('#register').click(function ()
        {
            ...
            $(".autoTable tbody tr").each(function (item, i)
            {
                ...
                saveReceipt(receiptID, "2", oldAmount, n, receiptDate, txt_Desc, txt_DocNumber, txt_Amount, txt_comment, "-1");
                $('span[id$=CurrentRemainAmountBox]').html(parseFloat(oldAmount) + parseFloat(txt_Amount));
                remainAmount += parseFloat(txt_Amount);
            });
            addAmountToGovernBoxByReceipt(sumAmount);
