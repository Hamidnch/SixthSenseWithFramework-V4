<%@ Control Language="C#" AutoEventWireup="true" CodeFile="RewardPaymentDocumentOperation.ascx.cs" Inherits="AdminPanel_AdminUserControl_RewardPaymentDocumentOperation" %>
...
<fieldset>
    <legend style="color: #ffcc00;">لیست اسناد پرداخت - صندوق جوایز</legend>
    <fieldset>
    ...
    <asp:UpdatePanel ID="UpdatePanel1" runat="server" UpdateMode="Conditional">
        <ContentTemplate>
            <fieldset>
                <asp:GridView ID="gv_rewardPaymentDocuments" runat="server" AutoGenerateColumns="False"
                    CssClass="GridViewStyle" HeaderStyle-CssClass="HeaderStyle" 
                    HeaderStyle-HorizontalAlign="Center" HeaderStyle-VerticalAlign="Middle" 
                    AllowPaging="True" onpageindexchanging="gv_rewardPaymentDocuments_PageIndexChanging" 
                    onrowcommand="gv_rewardPaymentDocuments_RowCommand" onrowdatabound="gv_rewardPaymentDocuments_RowDataBound">
...
<div id="divSelectUser" aria-labelledby="ui-dialog-title-dialog-form" role="dialog"
    ...
    <div style="width: auto; min-height: 0px; height: auto" class="ui-dialog-content ui-widget-content"
        ...
        <fieldset>
            ...
            <br />
            <br />
            <asp:DropDownList ID="ddl_SelectUser" CssClass="ddl_SelectUser" runat="server">
            </asp:DropDownList>
            <br />
                ...
                <ContentTemplate>
                    <div id="gridDiv">
                        <legend style="color: #262626;">
                            <table>
                                <tr>
                                    <td>شماره سند : <span id="Span2"><asp:Label id="lbl_PaymentDocID" runat="server"></asp:Label></span></td>
                                ...
                                <tr>
                                    <td colspan="4">
                                        ...
                                        <asp:TemplateField HeaderText="شماره مدرک" SortExpression="PaymentDoc_DocumentNumber">
                                            <ItemStyle Width="20%" VerticalAlign="Middle" HorizontalAlign="Center" />
                                            <ItemTemplate>
                                                <asp:Label ID="lbl_PaymentDoc_DocumentNumber" runat="server" Text='<%# PersianClass.getPersianNumberStyle(Eval("PaymentDoc_DocumentNumber", "{0}")) %>'></asp:Label>
                                            </ItemTemplate>
                                        </asp:TemplateField>
...
<div id="divFinanceOperation" aria-labelledby="ui-dialog-title-dialog-form" role="dialog"
    ...
    width: 800px; top: 82px; left: 460px;">
    <div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix">
        <span id="ui-dialog-title-dialog-form" class="ui-dialog-title">عملیات پرداخت</span><a
            role="button" class="ui-dialog-titlebar-close ui-corner-all" href="#"><span class="ui-icon ui-icon-closethick">بستن</span></a></div>
    <div style="width: auto; min-height: 0px; height: auto" class="ui-dialog-content ui-widget-content"
        id="dialog-Payment">
        <fieldset>
            <table>
                ...
                <tr><td colspan="4"><span>نام کاربر برنده: </span><span id="userNameSelected" style="color: Purple; font-weight: bold"></span></td></tr>
                <tr>
                ...
                <tr>
                    <td colspan="4">
                        ...
                        <input type="button" value="بازگشت" class="submit" onclick="$('#dialog-Payment').dialog('close');$('#dialog-selectUser').dialog('open');" />                        
                    </td>
                </tr>
            </table>
        </fieldset>
...
<script type="text/javascript">
    ...
    var n = 0; var sumAmount = 0; var flag;var selectUserID;
    function setBlock()
    {
        $.blockUI(
    ...
    function getRewardPaymentInfo(userID)
    {
        $.PageMethod('GetRewardPaymentInfo', onRewardPaymentInfoSuccess, onRewardPaymentInfoFail, 'userID', userID );
        function onRewardPaymentInfoSuccess(result)
        {
            var info = result.d;
        ...
        function onRewardPaymentInfoFail(response)
        {
            alert("خطای فراخوانی اطلاعات دریافت");
            $.unblockUI();
        }
    } 
    function reduceAmountFromRewardBoxByPaymentAndUserID(userID, amount)
    {
        setBlock();
        $.PageMethod('ReduceAmountFromRewardBoxByPaymentAndUserID', onReduceAmountSuccess, onReduceAmountFail, 'userID', userID, 'amount', amount);
        ...
        {
            alert("خطای کسر مبلغ از صندوق جایزه کاربر");
            $.unblockUI();
        }
    }
    ...
    $(document).ready(function ()
    {
        ...
        $("#dialog-Payment").dialog({
            ...
            show: 'clip', hide: 'clip', stack: true, position: 'center', title: 'سند پرداخت - صندوق جوایز', modal: true
        });
        ...
        $("#btn_SelectUser").click(function ()
        {
            ...
            selectUserID = $('select[id$=ddl_SelectUser] option:selected').val();
            selectUserName = $('select[id$=ddl_SelectUser] option:selected').text();
            selectUserID;
            getRewardPaymentInfo(selectUserID);
        ...
        $('#register').live('click', function (e)
        {
            ...
            if (currentAmount >= sumAmount)
            {
                $(".autoTable tbody tr").each(function (item, i)
                {
                    ...
                    savePayment(PaymentID, "4", oldAmount, n, PaymentDate, txt_Desc, txt_DocNumber, txt_Amount, txt_comment, selectUserID);
                    $('span[id$=CurrentRemainAmountBox]').html(parseFloat(oldAmount) - parseFloat(txt_Amount));
                    remainAmount -= parseFloat(txt_Amount);
                });
                reduceAmountFromRewardBoxByPaymentAndUserID(selectUserID, sumAmount);
