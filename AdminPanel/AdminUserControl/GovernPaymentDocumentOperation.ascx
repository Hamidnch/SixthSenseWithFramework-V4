<%@ Control Language="C#" AutoEventWireup="true" CodeFile="GovernPaymentDocumentOperation.ascx.cs" Inherits="AdminPanel_AdminUserControl_GovernPaymentDocumentOperation" %>
...
<fieldset>
    <legend style="color: #ffcc00;">لیست اسناد پرداخت - صندوق دولتی</legend>
    <fieldset>
    ...
    <asp:UpdatePanel ID="UpdatePanel1" runat="server" UpdateMode="Conditional">
        <ContentTemplate>
            <fieldset>
                <asp:GridView ID="gv_governPaymentDocuments" runat="server" AutoGenerateColumns="False"
                    CssClass="GridViewStyle" HeaderStyle-CssClass="HeaderStyle"
                    HeaderStyle-HorizontalAlign="Center" HeaderStyle-VerticalAlign="Middle" 
                    AllowPaging="True" 
                    onpageindexchanging="gv_governPaymentDocuments_PageIndexChanging" 
                    onrowcommand="gv_governPaymentDocuments_RowCommand" 
                    onrowdatabound="gv_governPaymentDocuments_RowDataBound">
            ...
            <asp:UpdatePanel ID="UpdatePanel2" runat="server" UpdateMode="Always">
                <ContentTemplate>
                    ...
                    <legend style="color: #262626;">                    
                        <table>
                            <tr>
                                <td>شماره سند : <span id="Span1"><asp:Label id="lbl_PaymentDocID" runat="server"></asp:Label></span></td>
                                <td>تاریخ ثبت : <span id="show_RegDate" runat="server"></span></td>
                                <td>نام کاربر ثبت : <span id="show_AutorName" runat="server"></span></td>                                
                            </tr>
                            <tr><td>شرح کلی : <span id="show_Comment" runat="server"></span></td></tr>
...
<script type="text/javascript">
    ...
    function getGovernPaymentInfo()
    {
        $.PageMethod('GetGovernPaymentInfo', onGovernPaymentInfoSuccess, onGovernPaymentInfoFail);
        function onGovernPaymentInfoSuccess(result)
        {
            var info = result.d;
        ...
        function onGovernPaymentInfoFail(response)
        {
            alert("خطا");
            $.unblockUI();
        }
    }
    function reduceAmountFromGovernBoxByPayment(amount)
    {
        setBlock();
        $.PageMethod('ReduceAmountFromGovernmentBoxByPayment', onReduceAmountSuccess, onReduceAmountFail, 'amount', amount);
    ...
    $(document).ready(function ()
    {
        ...
        $("#dialog-Payment").dialog({
            ...
            show: 'clip', hide: 'clip', stack: true, position: 'center', title: 'سند پرداخت - صندوق دولتی', modal: true
        ...
        $('#register').live("click", function (event)
        {
            ...
            if (currentAmount >= sumAmount)
            {
                $(".autoTable tbody tr").each(function (item, i)
                {
                    ...
                    savePayment(PaymentID, "2", oldAmount, n, PaymentDate, txt_Desc, txt_DocNumber, txt_Amount, txt_comment, "-1");
                    $('span[id$=CurrentRemainAmountBox]').html(parseFloat(oldAmount) - parseFloat(txt_Amount));
                    remainAmount -= parseFloat(txt_Amount);
                });
                reduceAmountFromGovernBoxByPayment(sumAmount);
