...
public partial class Sport_Sport_MelliRedirectUrl : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Context.User.Identity.IsAuthenticated)
        {
            try
            {
                x_trans_id = Request.Form["x_trans_id"].ToString();  //شماره رسيد الکترونيکي 
                x_response_code = Request.Form["x_response_code"].ToString();//1 = Approved, 2 = Declined, 3 = Error === نتيجه تراکنش مالي مشتري           
                x_response_subcode = Request.Form["x_response_subcode"].ToString();//کد جزيي پاسخ
                x_response_reason_code = Request.Form["x_response_reason_code"].ToString(); //کد علت پاسخ
                x_response_reason_text = Request.Form["x_response_reason_text"].ToString(); //توضيح علت پاسخ
                x_login = Request.Form["x_login"].ToString(); //شناسه پذیرنده
                x_fp_sequence = Request.Form["x_fp_sequence"].ToString(); //کد خرید یا شماره تراکنش
                x_fp_timestamp = Request.Form["x_fp_timestamp"].ToString();//زمان امضا
                x_amount = Request.Form["x_amount"].ToString(); //مبلغ خرید
                x_currency_code = Request.Form["x_currency_code"].ToString(); //واحد پول
                x_fp_hash = Request.Form["x_fp_hash"].ToString(); //امضا
                ...
                signature = BLL.HMAC_MD5("4n+UD+mdbfpP+DGS", result);
                if (signature.ToUpper() == x_fp_hash.ToUpper())
                {
                    ...
                    if (x_response_code == "1")
                    {
                        try
                        {
                            if (DAL.Sport_GetPaymentStatus(x_trans_id, x_fp_sequence, x_fp_timestamp) != "واریز")
                            {
                                try
                                {
                                    ...
                                    DAL.Sport_InsertIntoUserChargeBoxTurnover(userID, "واریز از طریق بانک ملی", float.Parse(x_amount),0, remain, "");
                                }
                                catch (Exception ex)
                                {
                                    BLL.SendEmail("AddPaymentAmountToChargeBox - بانك ملي", ex);
                                }
                            }
                        ...
                        {
                            lbl_Success.Text = "خطا در ثبت اطلاعات ارسالي از بانك ملي";
                            BLL.SendEmail("GetPaymentStatus - بانك ملي", ex);
                        }
                    }
                    ...
                    try
                    {
                        DAL.Sport_UpdatePeymentsMelli("1", x_trans_id, response_code.Text, x_fp_sequence, x_fp_timestamp, x_fp_hash,
                        ...
                        {
                            BLL.SendEmail("mail@hesesheshom.com", emailByUser, "پرداخت از طریق درگاه ملی", "عملیات پرداخت از طریق درگاه ملی با موفقیت انجام پذیرفت" +
                    ...
                    catch (Exception ex)
                    {
                        ...
                        BLL.SendEmail("خطا در هنگام بروز رسانی پرداخت کاربر در دیتابیس - بانک ملی", ex);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                BLL.SendEmail("خطا در اجرای عملیات بانک ملی", ex);
            }
        }
    }
}